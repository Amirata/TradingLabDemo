...
import {GrossAndNetForEachSymbol, Symbols} from "../../../../libs/definitions.ts";
...
const RangeSlider: React.FC<props> = ({sendToServer, startDate, endDate}) => {
    ...
    const getDateFromValue = (value: number): string => {
        const newDate = new Date(startDateObj);
        newDate.setDate(startDateObj.getDate() + value);
        return newDate.toISOString().split("T")[0];
    };

    const currentStartDate = getDateFromValue(range[0]);
    const currentEndDate = getDateFromValue(range[1]);
    ...
    useEffect(() => {
        ...
        if (!isInitialLoad) {
            timeoutRef.current = window.setTimeout(() => {
                sendToServer(currentStartDate, currentEndDate);
            }, 500);
        } else {
            setIsInitialLoad(false); // بعد از لود اولیه، این رو خاموش کن
        }
    ...
    }, [currentStartDate, currentEndDate, isInitialLoad]);
...
const BarChartCreate: React.FC<barChartProp> = ({data}) => {
    ...
    const chartData = data.map<BarChartData>((item) => {
        return {
            label: getEnumName(Symbols, Number(item.symbol)) as string,
            values:[
...
const GetGrossAndNetForEachSymbol = () => {

    const [isModalOpen, setIsModalOpen] = React.useState<boolean>(false);
    const [loading, setLoading] = React.useState(true);
    const [data, setData] = React.useState<GrossAndNetForEachSymbol[]>([]);
    const [years, setYears] = useState<{ startDate:string, endDate:string } | null>(null);
    const {id} = useParams<{ id: string }>();
    const {setSymbolList} = useAppStore();

    const sendToServer = (start: string, end: string) => {
        setLoading(true);
        apisWrapper.TradeAnalyseWrapper.getGrossAndNetForEachSymbol(id as string, start, end)
            .then((r) => {
                if(r){
                    setSymbolList(r.map(item => {
                        return item.symbol;
                    }));
                    setData(r);
                    setLoading(false);
    ...
    useEffect(() => {
        ...
        apisWrapper.TradeAnalyseWrapper.getGetTradeYears(id).then((yearsData) => {
            ...
            sendToServer(startDate, endDate); // لود اولیه اینجا انجام می‌شه
    ...
    const message = useMemo(() =>
            <div className="text-sm text-white/80 text-justify font-iran-sans-fa-num max-w-[600px]">
                این نمودار سود و زیان کلی را در بازه تاریخی به ازای هر نماد نشان می دهد.
            </div>
        , [])
        ...
        <div className="flex flex-col bg-zinc-700 rounded-sm">
            ...
            <div className="flex items-center pr-5 pt-5">
                <div className="flex items-center lg:basis-1/2">
                    <h2 className="font-iran-sans text-white/80 ">سود و زیان کلی به ازای هر نماد</h2>
            ...
            <div className="basis-1/2">
                {years &&
                    <RangeSlider sendToServer={sendToServer} startDate={years?.startDate} endDate={years?.endDate}/>}
            </div>
...
export default GetGrossAndNetForEachSymbol;