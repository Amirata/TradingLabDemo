...
import {formatDateTimeForm, formatDateTimeForJson} from "../../../../libs/utilities.ts";
...
export default function UpdateForm({dataPromise}: props) {
    ...
    const tradeData = use(dataPromise);
    ...
    const methods = useForm<FormData>({
        ...
        defaultValues: {
            id: tradeData.id,
            tradingPlanId: tradeData.tradingPlanId,
            symbol: tradeData.symbol,
            positionType: tradeData.positionType,
            volume: tradeData.volume.toString(),
            entryPrice: tradeData.entryPrice.toString(),
            closePrice: tradeData.closePrice.toString(),
            stopLossPrice: tradeData.stopLossPrice.toString(),
            entryDateTime: formatDateTimeForm(new Date(tradeData.entryDateTime)),
            closeDateTime: formatDateTimeForm(new Date(tradeData.closeDateTime)),
            commission: tradeData.commission.toString(),
            swap: tradeData.swap.toString(),
            pips: tradeData.pips.toString(),
            netProfit: tradeData.netProfit.toString(),
            grossProfit: tradeData.grossProfit.toString(),
            balance: tradeData.balance.toString(),
        },
    });
    const onSubmit = async (data: FormData) => {
        //console.log(data);
        await apisWrapper.TradeWrapper.update({
            id: tradeData.id,
    ...
    return (
        <>
            <div className="flex items-center justify-between p-5">
                <h1 className="text-white/80 font-iran-sans font-bold text-lg">ویرایش معامله از
                    پلن {tradeData.tradingPlanName} </h1>
                <BackBtn path="/"/>
            </div>
            <div className="p-5">
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                <ResetBtn
                                    resetfn={
                                        () => {
                                            methods.reset();
                                            methods.setValue("entryDateTime", formatDateTimeForm(new Date(tradeData.entryDateTime)), {shouldValidate: false});
                                            methods.setValue("closeDateTime", formatDateTimeForm(new Date(tradeData.closeDateTime)), {shouldValidate: false});
                                        }
                                    }/>
