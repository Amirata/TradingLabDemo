...
import {
  ...
  LABEL_MARGIN,
  NAMESPACE,
} from "../../libs/calendar/constants.ts";
import { useColorScheme } from "../../hooks/calendar/useColorScheme.ts";
import {
  loadingAnimationName,
  useLoadingAnimation,
} from "../../hooks/calendar/useLoadingAnimation.ts";
import { usePrefersReducedMotion } from "../../hooks/calendar/usePrefersReducedMotion.ts";
import {
  generateEmptyData,
  getClassName,
  groupByWeeks,
  range,
  validateActivities,
} from "../../libs/calendar/calendar.ts";
import {
  getMonthLabels,
  initWeekdayLabels,
  maxWeekdayLabelWidth,
} from "../../libs/calendar/label.ts";
import { createTheme } from "../../libs/calendar/theme.ts";
...
export const ActivityCalendar = forwardRef<HTMLElement, Props>(
  (
    ...
    function renderCalendar() {
      return weeks
        .map((week, weekIndex) =>
          week.map((activity:any, dayIndex:any) => {
            if (!activity) {
              return null;
            }
